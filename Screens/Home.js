    ...
    render() {
        ...
        return ( <
            ...
            renderItem = {
                ({ item }) =>
                <
                TouchableItem delayPressIn = { 50 }
                onPress = {
                    () => navigation.navigate('ItemDetails', { key: item.id }) }
                key = { item.id } >
                <
                View style = {
                    { margin: 11, alignItems: 'center', justifyContent: 'center' } } >
                ...
                <
                View style = {
                    { paddingHorizontal: 25, alignItems: 'center', justifyContent: 'center', flexDirection: 'row', position: 'absolute', backgroundColor: "#336699" } } >
                <
                Text style = {
                    { color: '#fff', fontSize: 24 } } > { item.price } & euro; < /Text> <
                Checkbox checked = { context.myCart.includes(item) }
                onCheck = {
                    (checked) => { this.handleToggleCbox(checked, item).then(r => null) } }
                /> <
                /View> <
                /ImageBackground> <
                Text style = {
                ...
                View style = {
                    { alignSelf: 'flex-start', maxWidth: 200, overflow: 'hidden' } } >
                <
                Text numberOfLines = { 2 }
                margin: 8,
                style = {
                    { fontSize: 14 } } > { item.description } < /Text> <
                /View> <
                /View> <
                /TouchableItem>
            ...
            /> <
            Button title = "Test Vider panier"
            icon = "delete"
            onPress = {
                () => {
